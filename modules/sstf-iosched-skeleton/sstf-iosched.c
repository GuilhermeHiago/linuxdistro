...
static int sstf_dispatch(struct request_queue *q, int force){
	...
	// deve iterar pela lista procurando request mais proxima
	struct list_head *pos;
	struct request *it_rq;
	
	// set rq p/ primeiro elemento (para depois comparar com outros elementos)
	rq = list_first_entry_or_null(&nd->queue, struct request, queuelist);
	long long unsigned distancia_atual = abs(last_pos - blk_rq_pos(rq));
	
	// caso lista não vazia
	if (rq){
		list_for_each(pos, &nd->queue){
				// pega valor do nodo atual
				it_rq = list_first_entry_or_null(pos, struct request, queuelist);
	
				long long unsigned distancia_it_rq = abs(last_pos - blk_rq_pos(it_rq));
	
				// verifica o mais proximo da ultima pos
				if(distancia_it_rq < distancia_atual){
					rq = it_rq;
					distancia_atual = distancia_it_rq;
				}
			}
	}
	
	if (rq) {
		// atualiza ultima posição escrita
